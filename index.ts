...
class LEBDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LEBDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawELBDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : LEBDNode {
        var curr : LEBDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineExpandBallDown {

    curr : LEBDNode = new LEBDNode(0)
    dir : number = 1
